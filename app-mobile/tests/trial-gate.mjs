#!/usr/bin/env node
// Testa o contador de trial local (2 prescrições grátis), a persistência
// do aviso legal e o gate que bloqueia a 3ª tentativa mostrando o
...
// 3) Limpar #2 (tentativa de uso 3) — bloqueado: paywall aparece, contador
//    não sobe de 2, campo não é limpo.
await page.fill('#peso', '9,0');
await page.click('#btnLimpar');
await page.waitForSelector('#overlayPaywall', { state: 'visible' });
count = await page.evaluate(() => window.localStorage.getItem('trialUsageCount'));
assert.equal(count, '2', 'contador não deveria passar de 2 quando bloqueado');
assert.equal(await page.inputValue('#peso'), '9,0', 'campo peso não deveria ter sido limpo quando bloqueado');

// 4) Assinante ativo (mock) ignora o contador mesmo com trial esgotado.
// O paywall ainda está visível do passo 3 (Task 2 não fecha o paywall no
